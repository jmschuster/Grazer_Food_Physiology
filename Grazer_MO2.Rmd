...
GrazerWeights<-GrazerWeights %>% dplyr::mutate(Treatment_Name = case_when(
  Treatment == 'Macrocystis' ~ 'Macrocystis pyrifera',
  Treatment == 'Saccharina' ~ 'Saccharina latissima',
  Treatment == 'Unfed' ~ 'No macroalgae'))
...
# SAVE FINAL PLOT
ggsave(filename="MassIndep_MR_V3",height=9, width=9, units="in", plot=grid.arrange(plot3a, plot3b, plot3c, plot3d+theme(legend.position='none'),legend2, ncol = 2,nrow=3,  widths = c(2.2, 2), heights = c(2.6, 2.9,0.3)), device="png")


####### NICE PLOT FOR ABSOLUTE MO2 ############
#### BUILDING SEPARATE PLOTS FOR EACH SPECIES

# Filling boxplots based on significance
CageExpSub <- CageExpSub %>% mutate(Signif_AbsMO2 = case_when(
  Species=='Mesocentrotus franciscanus' & Week=='Week 2' & Treatment=='Unfed' ~ 'Yes',
  Species=='Mesocentrotus franciscanus' & Week=='Week 2' & Treatment=='Saccharina' ~ 'No',
  Species=='Mesocentrotus franciscanus' & Week=='Week 2' & Treatment=='Macrocystis' ~ 'No',
  Species=='Mesocentrotus franciscanus' & Week=='Week 7' & Treatment=='Unfed' ~ 'Yes',
  Species=='Mesocentrotus franciscanus' & Week=='Week 7' & Treatment=='Saccharina' ~ 'No',
  Species=='Mesocentrotus franciscanus' & Week=='Week 7' & Treatment=='Macrocystis' ~ 'No',
  Species=='Strongylocentrotus purpuratus' & Week=='Week 2' & Treatment=='Unfed' ~ 'Yes',
  Species=='Strongylocentrotus purpuratus' & Week=='Week 2' & Treatment=='Saccharina' ~ 'No',
  Species=='Strongylocentrotus purpuratus' & Week=='Week 2' & Treatment=='Macrocystis' ~ 'No',
  Species=='Strongylocentrotus purpuratus' & Week=='Week 7' & Treatment=='Unfed' ~ 'Yes',
  Species=='Strongylocentrotus purpuratus' & Week=='Week 7' & Treatment=='Saccharina' ~ 'No',
  Species=='Strongylocentrotus purpuratus' & Week=='Week 7' & Treatment=='Macrocystis' ~ 'No',
  Species=='Pomaulax gibberosus' & Week=='Week 2' & Treatment=='Unfed' ~ 'Yes',
  Species=='Pomaulax gibberosus' & Week=='Week 2' & Treatment=='Saccharina' ~ 'No',
  Species=='Pomaulax gibberosus' & Week=='Week 2' & Treatment=='Macrocystis' ~ 'No',
  Species=='Pomaulax gibberosus' & Week=='Week 7' & Treatment=='Unfed' ~ 'Yes',
  Species=='Pomaulax gibberosus' & Week=='Week 7' & Treatment=='Saccharina' ~ 'No',
  Species=='Pomaulax gibberosus' & Week=='Week 7' & Treatment=='Macrocystis' ~ 'No'))

CageExpSub$Signif_AbsMO2<-as.factor(CageExpSub$Signif_AbsMO2)

plot4a<-CageExpSub %>% filter(rsqrt>=0.90 & Species=='Mesocentrotus franciscanus') %>%
  ggplot(aes(x=Week, y=MO2_ml.h, colour = Treatment_Name,fill=Signif_AbsMO2)) +
  geom_boxplot(outlier.shape=NA,lwd=1) +  # remove NAs
  xlab("Measurement time") +labs(y=bquote(' .\nM'*O[2]*~(mL~O[2] / h)*''))+ scale_fill_manual(values=c('white','gray90'),name='Treatment')+ scale_colour_manual(values=c("seagreen", "yellow4",'darkgrey'),name='Treatment')+scale_y_continuous(labels = formatter(nsmall = 2),limits=c(0,1.6))+theme_bw(base_size=15)+theme(legend.position='none',legend.margin=margin(t=-10),panel.grid.major = element_blank(),panel.border=element_blank(), axis.line= element_line(colour = "black",size=0.5),axis.title.x=element_blank(), panel.grid.minor = element_blank(),plot.title = element_text(face = "italic"))+labs(title="Mesocentrotus franciscanus",tag="A")+ annotate(geom="text", x=1, y=1.5, label="N = 98",color="black",size=5)+annotate(geom="text", x=2, y=1.55, label="N = 102",color="black",size=5) 

###

plot4b<-CageExpSub %>% filter(rsqrt>=0.90  & Species=='Strongylocentrotus purpuratus') %>%
  ggplot(aes(x=Week, y=MO2_ml.h, colour = Treatment_Name,fill=Signif_AbsMO2))+
  geom_boxplot(outlier.shape=NA,lwd=1) +  # remove NAs
  xlab("Measurement time") +labs(y=bquote(' .\nM'*O[2]*~(mL~O[2] / h)*''))+ scale_fill_manual(values=c('white','gray90'),name='Treatment')+ scale_colour_manual(values=c("seagreen", "yellow4",'darkgray'),name='Treatment')+scale_y_continuous(labels = formatter(nsmall = 2),limits=c(0,2.5))+theme_bw(base_size=15)+theme(legend.position='none',legend.margin=margin(t=-10),panel.grid.major = element_blank(),panel.border=element_blank(), axis.line= element_line(colour = "black",size=0.5),axis.title=element_blank(), panel.grid.minor = element_blank(),plot.title = element_text(face = "italic"))+ labs(title="Strongylocentrotus purpuratus",tag="B") + annotate(geom="text", x=1, y=2.4, label="N = 102",color="black",size=5)+annotate(geom="text", x=2, y=2.4, label="N = 105",color="black",size=5)


###

plot4c<-CageExpSub %>% filter(rsqrt>=0.90  & Species=='Pomaulax gibberosus') %>%
  ggplot(aes(x=Week, y=MO2_ml.h, colour = Treatment_Name,fill=Signif_AbsMO2))+
  geom_boxplot(outlier.shape=NA,lwd=1) +  # remove NAs
  xlab("Measurement time") +labs(y=bquote(' .\nM'*O[2]*~(mL~O[2] / h)*''))+ scale_fill_manual(values=c('white','gray90'),name='Treatment') +scale_colour_manual(values=c("seagreen", "yellow4",'darkgray'),name='Treatment')+ scale_y_continuous(labels = formatter(nsmall = 2),limits=c(0,1))+theme_bw(base_size=15)+ theme(legend.position='none',legend.margin=margin(t=-10),panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.border=element_blank(), axis.line= element_line(colour = "black",size=0.5), plot.title = element_text(face = "italic"))+labs(title="Pomaulax gibberosus",tag="C")+ annotate(geom="text", x=1, y=0.95, label="N = 95",color="black",size=5)+annotate(geom="text", x=2, y=0.98, label="N = 103",color="black",size=5)

###

plot4d<-CageExpSub %>% filter(rsqrt>=0.90  & Species=='Tegula pulligo') %>%
  ggplot(aes(x=Week, y=MO2_ml.h/N_Dusky, colour = Treatment_Name))+ 
  geom_boxplot(outlier.shape=NA,lwd=1) +  # remove NAs
  xlab("Measurement time") +labs(y=bquote(' .\nM'*O[2]*~(mL~O[2] / h)*''))+ scale_colour_manual(values=c("seagreen", "yellow4",'darkgray'),name='Treatment',labels=c("Higher quality macroalgae", "Lower quality macroalgae", "No macroalgae"))+ scale_y_continuous(labels = formatter(nsmall = 2), limits=c(0,0.10))+theme_bw(base_size=15)+ theme(panel.grid.major = element_blank(), axis.title.y=element_blank(), panel.grid.minor = element_blank(), panel.border=element_blank(), axis.line= element_line(colour = "black",size=0.5), legend.position=c(0.7,0.5), legend.direction = 'horizontal', legend.margin=margin(t=-10,l=150,b=-10), plot.title = element_text(face = "italic"))+labs(title="Tegula pulligo",tag="D")+ annotate(geom="text", x=1, y=0.09, label="N = 87",
              color="black",size=5)+ annotate(geom="text", x=2, y=0.09, label="N = 75",
              color="black",size=5)

legend2 <- get_legend(plot4d) # extract legend object

# SAVE FINAL PLOT
ggsave(filename="Absolute_MR_V3",height=9, width=9, units="in", plot=grid.arrange(plot4a, plot4b, plot4c, plot4d+theme(legend.position='none'),legend2, ncol = 2,nrow=3,  widths = c(2.2, 2), heights = c(2.6, 2.9,0.3)), device="png")
```
...
ggsave(filename="HeatResistance_Treatment_V2",height=5, width=7, units="in", plot=grid.arrange(plot_heat_Mfra,plot_heat_Spur, plot_heat_Pgib, plot_heat_Tpul+theme(legend.position='none'),legend, ncol = 2,nrow=3,  widths = c(2.2, 2), heights = c(2.6, 2.9,0.5)), device="png")
...
Mfra_W2<-aov(MO2_ml.h ~ Treatment, data = Mfra) # Week 2
Mfra_W7<-aov(MO2_ml.h ~ Treatment, data = Mfra7) # Week 7
...
# Week 2 and 7 violate normality

# Check for outliers
library(outliers)
# Week 2
grubbs.test(Mfra$MO2_ml.h) # outlier test; no outliers
Mfra<-Mfra %>% filter(MO2_ml.h<5.2) # remove outliers from df

# Week 7 (no outliers)
grubbs.test(Mfra7$MO2_ml.h) # outlier test
grubbs.test(Mfra7$MO2_ml.h, opposite=TRUE) # outlier test (lowest)

# Non-parametric test of treatment differences in Week 2
kruskal.test(MO2_ml.h ~ Treatment, data = Mfra) # KW test for multiple treatment levels

# Test which treatment levels are different:
wilcox_test(MO2_ml.h ~ Treatment, data = Mfra, p.adjust.method = "bonferroni")

# Non-parametric test of treatment differences in Week 7
kruskal.test(MO2_ml.h ~ Treatment, data = Mfra7) # KW test for multiple treatment levels

# Test which treatment levels are different:
wilcox_test(MO2_ml.h ~ Treatment, data = Mfra7, p.adjust.method = "bonferroni")
...
Spur_W2<-aov(MO2_ml.h ~ Treatment, data = Spur) # Week 2
Spur_W7<-aov(MO2_ml.h ~ Treatment, data = Spur7) # Week 7
...
library(outliers)
# Week 2
grubbs.test(Spur$MO2_ml.h) # outlier test; highest val is outlier
Spur<-Spur %>% filter(MO2_ml.h<6.7) # remove 1 outlier
# Week 7
grubbs.test(Spur7$MO2_ml.h) # outlier test
grubbs.test(Spur7$MO2_ml.h, opposite=TRUE) # outlier test (lowest)
Spur7<-Spur7 %>% filter(MO2_ml.h<4.7) # remove 1 outlier

# Tukeys post hoc for Week 2 (Week 2 Spur data meets assumptions after outlier is removed)
summary(Spur_W2)
TukeyHSD(Spur_W2, which = "Treatment")

# Tukeys post hoc for Week 7 (Week 7 Spur data meets assumptions after outlier is removed)
summary(Spur_W7)
TukeyHSD(Spur_W7, which = "Treatment")
...
Pgib_W2<-aov(MO2_ml.h ~ Treatment, data = Pgib) # Week 2
Pgib_W7<-aov(MO2_ml.h ~ Treatment, data = Pgib7) # Week 7
...
library(outliers)
# Week 2
grubbs.test(Pgib$MO2_ml.h) # outlier test
Pgib<-Pgib %>% filter(MO2_ml.h<0.83) # remove 14 outliers

# Week 7
grubbs.test(Pgib7$MO2_ml.h) # outlier test; repeat after removing each outlier IDd (one at a time)
grubbs.test(Pgib7$MO2_ml.h, opposite=TRUE) # outlier test (lowest)
Pgib7<-Pgib7 %>% filter(MO2_ml.h<1.3) # remove 5 outliers 

# Both W2 and W7 fail normality; even after removing outliers
# Non-parametric test of treatment differences; Week 2
kruskal.test(MO2_ml.h ~ Treatment, data = Pgib) # KW test for multiple treatment levels

# Test which treatment levels are different; Week 2
wilcox_test(MO2_ml.h ~ Treatment, data = Pgib, p.adjust.method = "bonferroni")

# Non-parametric test of treatment differences; Week 7
kruskal.test(MO2_ml.h ~ Treatment, data = Pgib7) # KW test for multiple treatment levels

# Test which treatment levels are different; Week 7
wilcox_test(MO2_ml.h ~ Treatment, data = Pgib7, p.adjust.method = "bonferroni")
...
Tpul$MO2_ml.h.Ind<-(Tpul$MO2_ml.h/Tpul$N_Dusky) # calc individual avrg MO2 (as multiple duskys per chamber)
Tpul7$MO2_ml.h.Ind<-(Tpul7$MO2_ml.h/Tpul7$N_Dusky) # calc individual avrg MO2 (as multiple duskys per chamber)

Tpul_W2<-aov(MO2_ml.h.Ind ~ Treatment, data = Tpul) # Week 2
Tpul_W7<-aov(MO2_ml.h.Ind ~ Treatment, data = Tpul7) # Week 7
...
# WEEK 2 & 7 MEETS NORMALITY ASSUMPTIONS; NO OUTLIERS

# Check for outliers where normality test failed
# Week 2
grubbs.test(Tpul$MO2_ml.h.Ind) # outlier test
Tpul<-Tpul %>% filter(MO2_ml.h.Ind<0.29) # remove 1 outlier
# Week 7 no outliers

# Parametric test for Week 7
summary(Tpul_W2)
TukeyHSD(Tpul_W2, which = "Treatment")
...
pS1<-CageExpSub %>% filter(rsqrt>=0.9 & Species=='Mesocentrotus franciscanus' & WetWeight_g<70) %>%
  ggplot(aes(x=Week, y=MO2_ml.h, fill = Treatment_Name)) + 
  geom_boxplot(outlier.shape=NA) +  # remove NAs
  xlab("Measurement time") +labs(y=bquote(' .\nM'*O[2]*~(mL~O[2] / h)*''))+ scale_fill_manual(values=c("seagreen", "yellow4",'seashell3'),name='Treatment',labels=c("Higher quality macroalgae", "Lower quality macroalgae", "No macroalgae"))+scale_y_continuous(labels = formatter(nsmall = 2),limits=c(0,1.6))+theme_bw(base_size=15)+theme(plot.title = element_text(face='italic'),legend.position=c(0.7,0.5), legend.direction = 'horizontal',legend.margin=margin(t=-10,l=150,b=-10),panel.border=element_blank(),axis.line= element_line(colour = "black",size=0.5),panel.grid.major = element_blank(), panel.grid.minor = element_blank())+labs(title="Mesocentrotus franciscanus",tag="A")+ annotate(geom="text", x=1, y=1.4, label="N = 96",color="black",size=5)+annotate(geom="text", x=2, y=1.5, label="N = 97",color="black",size=5) 
...
# 96 individuals remain in subset for sensitivity analysis !!
...
# Stats for absolute MO2 of subset:
Mfra_Sub<-aov(MO2_ml.h ~ Treatment, data = subset(Mfra_all,rsqrt>=0.9 & Week=='Week 2' & WetWeight_g<70)) # Week 2
Mfra_Sub7<-aov(MO2_ml.h ~ Treatment, data = subset(Mfra_all,rsqrt>=0.9 & Week=='Week 7' & WetWeight_g<70)) # Week 2

# Shapiro-Wilko test for normality
aov_residuals <- residuals(object = Mfra_Sub)
shapiro.test(x = aov_residuals ) # p < 0.05, normality is violated

# Week 2 and 7 subset violates normality

# Non-parametric test of treatment differences Week 2
kruskal.test(MO2_ml.h ~ Treatment, data = subset(Mfra_all,Common_Name=='Red sea urchin' & rsqrt>=0.9 & Week=='Week 2' & WetWeight_g<70)) # KW test for multiple treatment levels; WEEK 2

kruskal.test(MO2_ml.h ~ Treatment, data = subset(Mfra_all,Common_Name=='Red sea urchin' & rsqrt>=0.9 & Week=='Week 7' & WetWeight_g<70)) # KW test for multiple treatment levels; WEEK 7

# Test which treatment levels are different:
wilcox_test(MO2_ml.h ~ Treatment, data = subset(Mfra_all,Common_Name =='Red sea urchin' & rsqrt>=0.9 & Week=='Week 2' & WetWeight_g<70),p.adjust.method = "bonferroni") # WEEK 2

wilcox_test(MO2_ml.h ~ Treatment, data = subset(Mfra_all,Common_Name =='Red sea urchin' & rsqrt>=0.9 & Week=='Week 7' & WetWeight_g<70),p.adjust.method = "bonferroni") # WEEK 7
...
pS2<-CageExpSub %>% filter(rsqrt>=0.9 & Species=='Pomaulax gibberosus' & WetWeight_g<40) %>%
  ...
  xlab("Measurement time") +labs(y=bquote(' .\nM'*O[2]*~(mL~O[2] / h)*''))+ scale_fill_manual(values=c("seagreen", "yellow4",'seashell3'),name='Treatment')+theme_bw(base_size=15)+theme(plot.title = element_text(face='italic'),legend.position='bottom',legend.margin=margin(t=-10),panel.border=element_blank(),axis.line= element_line(colour = "black",size=0.5),panel.grid.major = element_blank(), panel.grid.minor = element_blank())+scale_y_continuous(labels = formatter(nsmall = 2),limits=c(0,1))+labs(title="Pomaulax gibberosus",tag="B")+ annotate(geom="text", x=1, y=0.9, label="N = 76",color="black",size=5)+annotate(geom="text", x=2, y=0.95, label="N = 78",color="black",size=5) 
...
Pgib_Sub<-aov(MO2_ml.h ~ Treatment, data = subset(Pgib_all,rsqrt>=0.9 & Week=='Week 2' & WetWeight_g<40)) # Week 2
Pgib_Sub7<-aov(MO2_ml.h ~ Treatment, data = subset(Pgib_all,rsqrt>=0.9 & Week=='Week 7' & WetWeight_g<40)) # Week 7

# Shapiro-Wilko test for normality
aov_residuals <- residuals(object = Pgib_Sub)
shapiro.test(x = aov_residuals ) # p < 0.05, normality is violated

# Week 2 & 7 subset violates normality


# Non-parametric test of treatment differences (Week 2 & 7)
kruskal.test(MO2_ml.h ~ Treatment, data = subset(Pgib_all,Common_Name=='Red turban snail' & rsqrt>=0.9 & Week=='Week 2' & WetWeight_g<40))

kruskal.test(MO2_ml.h ~ Treatment, data = subset(Pgib_all,Common_Name=='Red turban snail' & rsqrt>=0.9 & Week=='Week 7' & WetWeight_g<40)) # KW test for multiple treatment levels

# WEEK 7 significantly different MO2 across treatments in red turban snails

# Test which treatment levels are different:
wilcox_test(MO2_ml.h ~ Treatment, data = subset(Pgib_all,Common_Name =='Red turban snail' & rsqrt>=0.9 & Week=='Week 2' & WetWeight_g<40),p.adjust.method = "bonferroni")

wilcox_test(MO2_ml.h ~ Treatment, data = subset(Pgib_all,Common_Name =='Red turban snail' & rsqrt>=0.9 & Week=='Week 7' & WetWeight_g<40),p.adjust.method = "bonferroni")
...
ggsave(filename="SensitivityTest_MO2_V2.png",height=4, width=9, units="in", plot=grid.arrange(pS1+theme(legend.position='none'),pS2+theme(legend.position='none'),legend1, ncol = 2,nrow=2, widths = c(2.7, 2.7), heights = c(2.5,0.3)),device="png")
